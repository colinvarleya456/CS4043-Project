...
#Parts of the game that can be referenced
@export var player_cam : Camera3D
@onready var animation_player: AnimationPlayer = $AnimationPlayer
@onready var player_collision: CollisionShape3D = $playerCollision
@onready var interact_cast: RayCast3D = $Neck/Camera3D/interactCast
@onready var crouch_cast: ShapeCast3D = $crouchCast

#Can change variables in Godot Editor
...
@export var Bomb: PackedScene = load("res://Scenes/grenade.tscn")

#Loads player info at the start
func _ready() -> void:
	assignPlayerInfo()

#Constantly checks position every frame and if certain keys are being pressed, Eg: Movement
...
func grenadethrow():
	if Input.is_action_just_released("t"):
		print("A")
		var grenadeins = Bomb.instantiate()
		print("B")
		grenadeins.position = $Neck/grenadePos.global_position
		...
		print("C")
		grenadeins.apply_central_impulse(PlayerRotation * force + Vector3(0, upDirection, 0))
		print("D")
...
# Cycles between weapon and nothing
func weaponcycle():
	if Input.is_action_pressed("weapon cycle"):
		pass
	
#Checks to see if lmb is being held and if it is shoots.
...
#Can remove this cause of mouse lock by pressing v
...
#Press control to crouch or uncrouch
...
#Gives all player informaton like height and state
...
#Checks to see if something is interactable
func interactFunc():
	interact_cast.force_raycast_update()
	if interact_cast.is_colliding():
		interact_cast.get_collider().emit_signal("interact", self)

#More things that can be changed in godot directly
